 ...
 *
 * // Condition will be 'name = `forum_home_url` AND module = `forum`'
 * Yii::app()->settings->forum->forum_home_url  
 * // Condition will be 'name = `forum_home_url`'
 * Yii::app()->settings->main->forum_home_url 
 * // Will throw Exception because doesn't exists in condition 'module = `blog`' 
 * Yii::app()->settings->blog->forum_home_url  
 *
...
class Settings extends CApplicationComponent
{
    ...
    public function __get($app)
    {
        if ($this->applications && array_key_exists($app, $this->applications)) {
            self::$models[$app] = new ApplicationSetting($this->makeConfig($app));
            return self::$models[$app];
        } else {
    ...
    private function makeConfig($app)
    {
        ...
        $this->applications[$app]['settingsCategoryColumn'] : $this->settingsCategoryColumn;
        ...
        $this->applications[$app]['connection'] : $this->connection;
        
        $settingsTable = isset($this->applications[$app]['settingsTable']) ?
        $this->applications[$app]['settingsTable'] : $this->settingsTable;
        
        $settingsIdColumn = isset($this->applications[$app]['settingsIdColumn']) ?
        $this->applications[$app]['settingsIdColumn'] : $this->settingsIdColumn;
        
        $settingsValueColumn = isset($this->applications[$app]['settingsValueColumn']) ?
        $this->applications[$app]['settingsValueColumn'] : $this->settingsValueColumn;
