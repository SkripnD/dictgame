 ...
 */
class PRedis extends CApplicationComponent
{
    const DB_DICTIONARY      = 0;
    ...
    {
        if ($this->client === null || $reconnect) {
            $this->client = new Redis;
            if ($this->unixSocket !== null) {
                $this->client->connect($this->unixSocket);
            } else {
                $this->client->connect($this->hostname, $this->port, $this->timeout);
            }
            if (isset($this->password)) {
                if ($this->client->auth($this->password) === false) {
    ...
    {
        if (isset($this->getClient()->socket)) {
            $this->getClient()->select($db);
        }
